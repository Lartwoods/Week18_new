const superheroes = document.querySelector(".main__cards");
const heroes = [
  {
    name: "Бетмен",
    universe: "DC Comics",
...
const clickButton = (elem) => {
  const rating = elem.value;
  const parent = elem.parentElement.id.split("_")[1];
  localStorage.setItem(heroes[parent].name, rating);
};
const heroesHtml = heroes.map((hero, index) => {
  return `<div class="main-card">
  <h2 class="main__cards_head">${hero.name}</h2>
  <p class="main__cards_universe">${hero.universe}</p>
  <p class="main__cards_alterego">${hero.alterego}</p>
  <p class="main__cards_occupation">${hero.occupation}</p>
  <p class="main__cards_friends"> Союзники: ${hero.friends}</p>
  <p class="main__cards_superpowers">Суперспособности: ${hero.superpowers}</p>
  <img
    class="main__cards_image"
    src=${hero.url}
    alt="Product Image"
  />
  <p class="main__cards_descr">Информация: ${hero.info}</p>
  <div id="rating_${index}" class="rating" >
  <button class="star-button" onClick="clickButton(this)" value ="1">★</button>
  <button class="star-button" onClick="clickButton(this)" value ="2">★</button>
  <button class="star-button" onClick="clickButton(this)" value ="3">★</button>
  <button class="star-button" onClick="clickButton(this)" value ="4">★</button>
  <button class="star-button" onClick="clickButton(this)" value ="5">★</button>
</div>
<div class="mark"></div>
</div>
`;
...
let content = "";
heroesHtml.forEach((hero) => {
  content += hero;
});
superheroes.innerHTML = content;
